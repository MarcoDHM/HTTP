...
function fetchAllRickMortyCharacters() {
    const fetchPromises = [];
    for (let i = 1; i <= 42; i++) {  // 42 páginas en la API de Rick and Morty
        fetchPromises.push(fetch(`${rickAndMortyApiUrl}${i}`).then(res => res.json()));
    }
...
function displayPokemonPage(page) {
    ...
    charactersToDisplay.forEach((character, index) => {
        ...
        characterDiv.innerHTML = `
            ...
            <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${startIndex + index + 1}.png" alt="${character.name}">
        `;
        pokemonContainer.appendChild(characterDiv);
    });
    document.getElementById('pokemon-page-info').innerText = `Página ${page} de ${totalPokemonPages}`;
}
...
function displayRickAndMortyPage(page) {
    const startIndex = (page - 1) * 20;
    const endIndex = startIndex + 20;
    const charactersToDisplay = allRickMortyCharacters.slice(startIndex, endIndex);
    ...
    document.getElementById('rick-morty-page-info').innerText = `Página ${page} de ${totalRickMortyPages}`;
}
...
pokemonSearchInput.addEventListener('input', () => {
    ...
    if (query === '') {
        ...
        const filteredCharacters = allPokemonCharacters.filter(character => character.name.toLowerCase().includes(query));
...
rickMortySearchInput.addEventListener('input', () => {
    ...
    if (query === '') {
        ...
        // Mostrar resultados filtrados de todas las páginas
        const filteredCharacters = allRickMortyCharacters.filter(character => character.name.toLowerCase().includes(query));
        filteredCharacters.forEach((character, index) => {
            const characterDiv = document.createElement('div');
            characterDiv.classList.add('character');
            characterDiv.style.setProperty('--i', index);
